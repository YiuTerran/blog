...
4. 优化资源占用和满足离线自治。这也是其他方案都有的能力，不再赘述；
...
7. 边缘端与k8s生态不兼容；只有少量开源项目支持kubeEdge，如kubesphere. 一般开源软件只兼容原生k8s.
...
* 性能未知，官方暂未给出相关数据；kubeEdge有10w边缘节点的数据（但是没有多设备压测的数据）；

#### 其他

还有superEdge等开源产品，不过成熟度不够高，这里不再赘述。

## 监控方案

虽然上面这些方案都有一套自己提供的监控，但是都不够完善。

这里仍然统一使用之前提的ElasticAgent + Elastic APM方案即可，支持所有环境（k8s/非k8s）的可观测性数据收集。
...
墨斗服务iot部分虽然跑在k8s里，但是和云原生其实没啥关系。上面kubeEdge的方案其实就是一种云原生设计，但是设计的并不好。

实际上云原生属于后端开发最前沿，同时也是最激进的技术，现在并不成熟。之前说过UDP/SCTP服务就很难迁移到k8s里，因为支持的不好。而且java语言现阶段对云原生的适配本来就不太好，可能spring native等技术发展起来、java协程技术成熟之后，java才能更适配云原生环境。

在框架方面，springCloud和云原生基本没半毛钱关系，好处是可以跨云与非云通信，坏处是只支持java.

目前流行的框架主要是istio和dapr，其中后者甚至可以在非容器环境运行。个人认为还是不要太迷恋k8s，服务强依赖k8s并不一定是什么好事。dapr的设计至少在当下还是更成熟稳健一些。所以，采用springboot+dapr改写iot服务，并将视频云、视频会议等服务改为go+dapr，是比较好的开端。

dapr的设计思路是将软件开发中的常用组件和常用模式加了一层抽象，这样我们的服务无需访问redis/kafka/nacos这种中间件，而是通过dapr来调用（因此会造成性能损耗）；同时应用无需关心可观测性问题，因为这些都在dapr中统一处理了。dapr可以运行在k8s外，开发者调试非常方便；而生产环境的dapr在k8s里运行在sidecar里，这样你的服务自动就变为云原生了。
