...
国标GB28181（以下简称国标）文档分析的文档我去年其实写过一版，但是由于原来的电脑炸了，文档还没来得及传上去就丢了（痛苦），所以还要重写一版做笔记。
...
RFC3261规定了SIP协议的基础内容，包括`Register`, `Invite`等一系列注册和通信机制。SIP协议的通信方式类似HTTP，是典型的请求-响应。
...
以上的整个流程称之为一个`Dialog`，这个流程被称为`Session Model`通话模式。消息体负载大于1300字节时，一般采用该种通信模式。这也是RFC3261最初规定的一种通信方式。

### Pager Model

另外一种通信模式被称为`Pager Model`，适用于小流量通信。**RFC3428**扩展了SIP协议，增加了`MESSAGE`方法，类似UDP的设计思路，直接构造一条消息发过去即可（格式一般是SDP或者CPIM）。

![Pager Model](https://s2.loli.net/2022/03/02/meZKJFOcr7MYw4T.png)
...
对于Dialog，From tag, To tag, Call id三者，就形成了本次会话的唯一ID，既dailog id.

在SIP消息最初发出的时候，发送端(UAC)会在`From`后面生成tag，但是To后面没有。消息接收端(UAS)会在回复的时候在`To`后面加上tag。

`CSeq`头用于保证消息的顺序性，在同一个Dialog中标识及排序事务（transaction）以及区分新的请求与请求的重发。对于ACK而言，CSeq的序号必须与其所对应的request相同。对于CANCEL而言，CSeq的序号也必须与其cancel掉的request相同。否则，Dialog里面每个新的Request，Cseq+1。需要注意的是：在同一个对话中的UAC和UAS分别维护自己的CSeq序号，他们发出请求的CSeq序号是不相关的。

一次呼叫只能建立一个会话，但是可以建立多个dialog，因为呼叫可以是多个目标（会议），只是每个dialog的Request和Response的Call-ID必须一致。

`Via`中的`branch`是一个事务的id，这个branch参数的值必须用**z9hG4bK**打头。其它部分是对“To, From, Call-ID头域和Request-URI”按一定的算法加密后得到。
